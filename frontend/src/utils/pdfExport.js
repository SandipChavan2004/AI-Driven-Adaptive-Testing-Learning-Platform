...
export const downloadTestReport = async (opts) => {
  const {
    ...
    level, userName, date, recommendations = [], elementId,
  } = opts;
  ...
  pdf.text('VL', 16.5, 21.5);

  /* ── Brand name ── */
  pdf.setTextColor(255, 255, 255);
  pdf.setFontSize(18);
  pdf.setFont('helvetica', 'bold');
  pdf.text('VantageLearn', 33, 16);
  pdf.setFontSize(9);
  pdf.setFont('helvetica', 'normal');
  pdf.text('Adaptive Learning Platform', 33, 23);
  ...
  pdf.text('Generated by VantageLearn · Adaptive Learning Platform', W / 2, H - 9, { align: 'center' });
  pdf.text(`Score ${score}/100 · ${subject} · ${date}`, W / 2, H - 4, { align: 'center' });

  /* ── Download ── */
  pdf.save(`VantageLearn_${subject}_Report_${date.replace(/\//g, '-')}.pdf`);
};
